...
**Setup (once).** Add two repository secrets under *Settings → Secrets and variables → Actions* — Docker Scout requires a (free) Docker account:
...
**Build & delivery** — runs by itself on `git push origin main`. Check *Actions → Publish Docker Image*; the image appears under *Packages*.

**Security scan** — *Actions → SBOM/SCA Scout → Run workflow*, with:

```
ghcr.io/tasosm12/firstr-devsecops:latest
```

> ⚠️ The input's `default:` still points at `ghcr.io/tasosm12/algoquest:latest` (old name). Left as-is, `docker pull` fails.

**Results** — CVE table and base-image recommendations in the run log; `sca_scout.sarif` and `sbom_scout.spdx.json` as run artifacts; alerts under *Security → Code scanning* (`Security/SCA`).

**Scan locally**, before pushing:

```bash
docker scout quickview        ghcr.io/tasosm12/firstr-devsecops:latest
docker scout cves             ghcr.io/tasosm12/firstr-devsecops:latest
docker scout recommendations  ghcr.io/tasosm12/firstr-devsecops:latest
```

**Automate the scan on every build** — add to `scout.yml`:
...
and reference the image as `${{ github.event.inputs.docker_image || 'ghcr.io/tasosm12/firstr-devsecops:latest' }}`.
...
| Improvement | How |
|---|---|
| Multi-stage build (JRE runtime, no compiler shipped) | `FROM ...jdk AS build` → `COPY --from=build` into `...jre-jammy` |
| Non-root runtime | `RUN useradd -m app` + `USER app` |
| Pin the base image | `FROM eclipse-temurin:17-jdk-jammy@sha256:...` |
| Traceable tags | `tags: ${{ env.IMAGE }}:latest,${{ env.IMAGE }}:${{ github.sha }}` |
| Security gate | `only-severities: critical,high` + `exit-code: true` |
| Scheduled rescans | `on: schedule: - cron: '0 6 * * 1'` |
| Second opinion | Trivy or Grype alongside Scout |
| Provenance & signing | `provenance: true`, `sbom: true`; Cosign |
| Pin actions to SHA | `actions/checkout@a1b2c3…` instead of `@v4` |
...
Ships only the JRE and the compiled classes: no compiler, no sources, no build tools — fewer packages, fewer CVEs, smaller attack surface.
